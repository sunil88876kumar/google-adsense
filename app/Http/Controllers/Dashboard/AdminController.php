...
class AdminController extends Controller
{
    ...
    public function googleAdsenseNewOrEdit($id = null){
        if ($id == null){
            $googleAdsenseAdNeworEdit = null;
        }else{
            $googleAdsenseAdNeworEdit = AdsenseAd::where('id', $id)->firstOrFail();
        }
        return view('panel.admin.adsense.googleAdsenseAdNeworEdit', compact('googleAdsenseAdNeworEdit'));
    }

    public function googleAdsenseSave(Request $request, $id = null){
        ...
        if ($id) {
            $adsenseAd = AdsenseAd::findOrFail($id);
            $adsenseAd->update($validatedData);
        } else {
            $adsenseAd = new AdsenseAd($validatedData);
            $adsenseAd->save();
        }
        return redirect()->route('dashboard.admin.googleadsense.index')->with('success', 'Google Adsense Ad saved successfully.');
    }
